...
const BookForm = ({ onBookAdded }) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      setFormValid(title !== '' && author !== '' && pageCount !== '' && dateRead !== '');
    }, 500);
    ...
    <form>
      <div className='new-book-controls'>
        <div className='new-book-control'>
          <label>Título</label>
          <input onChange={changeTitleHandler} type='text' className='input-control' />
        </div>
        <div className='new-book-control'>
          <label>Autor</label>
          <input onChange={changeAuthorHandler} type='text' className='input-control' />
        </div>
        <div className='new-book-control'>
          <label>Páginas</label>
          <input onChange={changePageCountHandler} type='number' className='input-control' min='1' step='1' />
        </div>
        <div className='new-book-control'>
      ...
      <div className='new-book-actions'>
        ...
        <Button disabled={!formValid} onClick={addBookHandler} variant='primary'>
          Agregar lectura
        </Button>
      </div>
    </form>
